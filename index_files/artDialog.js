...
(function() {
	...
	*/
	...
	z = 999999,//对话框初始叠加高度(重要：Opera浏览器z-index的最大限制都小于其他浏览器)
	...
	newBox = function() {
		...
		$.zIndex = function(o) {
			var x = o ? o : ui_dialog;
			x.style.zIndex = ++z;
			ui_dialog_wrap.style.zIndex = ++z;//IE6与Opera叠加高度受具有绝对或者相对定位的父元素z-index控制的
			return $;
		};
		...
		$.html = function(title, url, content, yesFn, noFn, yesText, noText, style, closeFn, time, id) {
			...
			if (content) {
				...
				var s = function(){
					f.style.visibility = 'visible';//显示iframe
				};
				if (f.attachEvent){
		...
		$.align = function(width, height, left, top, fixed, lock, c) {
			...
			if (C.l < 0) C.l = 0;
			if (C.t < 0) C.t = 0;
			if (ui_dialog_wrap.id == hideId) C.l = '-9999999';//让预加载背景图的对话框偏离可视范围
